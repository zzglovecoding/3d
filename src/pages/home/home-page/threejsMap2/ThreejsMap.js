...
class ThreejsMap {
     ...
     * @desc 绘制shape
     */
    drawShape(points, set = {}) {
        const { color, opacity } = Object.assign({...this.set.mapStyle}, set); // util.getRandomColor(); // 颜色
    ...
    drawMap() {
            ...
            d.vertor3.forEach(v1 => {
                if (v1[0] instanceof Array) {
                    v1.forEach(v2 => {
                        ...
                        // 绘制阴影
                        const shadow = this.drawShape(v2, { opacity: 0.2, color: '#0080ff' });
                        gShadow.add(shadow);
                    });
                } else {
                    ...
                    // 绘制阴影
                    const shadow = this.drawShape(v1, { opacity: 0.2, color: '#0080ff' });
                    gShadow.add(shadow);
                }
            });
            g.data = d;
        ...
        gShadow.position.set(0, 0, -this.set.amount);
        this.scene.add(gShadow);
